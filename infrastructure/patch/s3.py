...
class S3(Storage):
    ...
    def get_flow(self, flow_location: str) -> "Flow":
        ...
        try:
            self.logger.info("GSD START Downloading {} from {}".format(flow_location, self.bucket))
            self._boto3_client.download_fileobj(
                Bucket=self.bucket, Key=flow_location, Fileobj=stream
            )
            self.logger.info("GSD END Downloading {} from {}".format(flow_location, self.bucket))
        except ClientError as err:
            self.logger.error("Error downloading Flow from S3: {}".format(err))
            self.logger.error("GSD Error downloading Flow from S3: {}".format(err), exc_info=err)
            raise err
        ...
        self.logger.info("GSD END stream read, cloudpickle.load")

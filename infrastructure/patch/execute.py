...
def cloud_flow():
    ...
    try:
        ...
        with prefect.context(secrets=secrets, loading_flow=True):
            print(f'GSD - execute.py storage get flow')
            flow = storage.get_flow(storage.flows[flow_data.name])
            print(f'GSD - DONE execute.py storage get flow')
            environment = flow.environment

            print(f'GSD - execute.py flow data env = {environment}')
            environment.setup(flow)
            print(f'GSD - execute.py flow setup done')
            environment.execute(flow)
            print(f'GSD - execute.py flow execute done')
